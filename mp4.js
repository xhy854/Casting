...
var MP4Player = (function reader() {
  ...
  function constructor(stream, useWorkers, webgl, render) {
    console.log("MP4Player.constructor.in");
    this.stream = stream;
    ...
    this.avc.onPictureDecoded = function(){
      console.log("onPictureDecoded.in")
  ...
  function updateStatistics() {
    console.log("updateStatistics.in");
    ...
    console.log("updateStatistics.in.s.videoPictureCounter="+s.videoPictureCounter);
    console.log("updateStatistics.in.s.windowPictureCounter="+s.windowPictureCounter);
  ...
  constructor.prototype = {
    readAll: function(callback) {
      ...
      this.stream.readAll(null, function (buffer) {/*
        ...
        */
        this.isRead = true;
        this.H264ByteStream = new Uint8Array(buffer);
        if (callback)
          callback();
      }.bind(this));
    },
    play: function() {
      ...
      */
      ...
      //this.avc.decode(sps); //debao did
      //this.avc.decode(pps); //debao did

      /* Decode Pictures */
      var pic = 0;
      //var H264Nal = this.H264ByteStream;
      var H264NalFrame = framesList.shift();
      console.log("avc decode from debao");
      this.avc.decode(H264NalFrame);
      /*setTimeout(function foo() {
        var avc = this.avc;
       video.getSampleNALUnits(pic).forEach(function (nal) {
          console.log("avc decode pic:"+pic);
          avc.decode(nal);
        });
        pic ++;
        if (pic < 3000) {
...
var Broadway = (function broadway() {
  function constructor(div) {
    console.log("Broadway constuctor.in");
    ...
    if (div.attributes.webgl){
      if (div.attributes.webgl.value == "true"){
        console.log("div.attributes.webgl.value=true");
        webgl = true;
      };
      if (div.attributes.webgl.value == "false"){
          console.log("div.attributes.webgl.value=false");
        webgl = false;
      };
    };
    ...
    this.canvas.onclick = function () {
      //this.play();
        console.log("onclick.x="+event.pageX+";y="+event.pageY);
    }.bind(this);

    this.canvas.onmousemove = function(event) {
        if (!isMouseOpen)
            return;
        //sendMouseEvent("mouse:mouseMove:"+(event.pageX-10)+":"+(event.pageY-10));
        sendMouseEventBin(MOU_STATUS_MOVED, event.pageX-10, event.pageY-10, 0);
    };
    ...
    this.canvas.onmousedown = function(event) {
        console.log("onmousedown.x="+event.pageX+";y="+event.pageY+"button="+event.button);
        if (!isMouseOpen)
          return;
        switch (event.button) {
            case 0:
              //sendMouseEvent("mouse:mouseLeftDown:"+(event.pageX-10)+":"+(event.pageY-10));
              sendMouseEventBin(MOU_STATUS_B1DOWN, event.pageX-10, event.pageY-10, 0);
              return;
            case 1:
              //sendMouseEvent("mouse:mouseWheelDown:"+(event.pageX-10)+":"+(event.pageY-10));
              return;
            case 2:
              //sendMouseEvent("mouse:mouseRightDown:"+(event.pageX-10)+":"+(event.pageY-10));
              sendMouseEventBin(MOU_STATUS_B2DOWN, event.pageX-10, event.pageY-10, 0);
              return;
            default:
    ...
    this.canvas.onmouseup = function(event) {
        console.log("onmouseup.x="+event.pageX+";y="+event.pageY+"button="+event.button);
        if (!isMouseOpen)
            return;
        switch (event.button) {
            case 0:
                //sendMouseEvent("mouse:mouseLeftUp:"+(event.pageX-10)+":"+(event.pageY-10));
                sendMouseEventBin(MOU_STATUS_B1UP, event.pageX-10, event.pageY-10, 0);
                return;
            case 1:
                //sendMouseEvent("mouse:mouseWheelUp:"+(event.pageX-10)+":"+(event.pageY-10));
                return;
            case 2:
                //sendMouseEvent("mouse:mouseRightUp:"+(event.pageX-10)+":"+(event.pageY-10));
                sendMouseEventBin(MOU_STATUS_B2UP, event.pageX-10, event.pageY-10, 0);
                return;
            default:
                console.log("unknown mouse event");
                return;
    ...
    this.canvas.onmousewheel = function(event) {
      console.log("onmousewheel.delta="+event.wheelDelta);
      if (!isMouseOpen)
        return;
      if (event.wheelDelta > 0) {
          //sendMouseEvent("mouse:mouseWheelUp:" + (event.pageX - 10) + ":" + (event.pageY - 10));
          sendMouseEventBin(MOU_STATUS_B3UP, event.pageX-10, event.pageY-10, 0);
          event.preventDefault();
          return false;
      }
      else if (event.wheelDelta <0) {
          //sendMouseEvent("mouse:mouseWheelDown:" + (event.pageX - 10) + ":" + (event.pageY - 10));
          sendMouseEventBin(MOU_STATUS_B3DOWN, event.pageX-10, event.pageY-10, 0);
          event.preventDefault();
          return false;
      }
    };
    ...
    document.onkeydown = function(event) {
      //console.log("keyEvent.onkeyDown.keyCode="+event.keyCode+";code="+event.code+";key="+event.key);
      if (!isMouseOpen)
          return;
      InputEvt(event);
    };
    document.onkeypress = function(event) {
      //console.log("keyEvent.onkeypress.keyCode="+event.keyCode+";code="+event.code+";key="+event.key);
      if (!isMouseOpen)
          return;
      InputEvt(event);
    };
    document.onkeyup = function(event) {
      //console.log("keyEvent.onkeyup.keyCode="+event.keyCode+";code="+event.code+";key="+event.key);
      if (!isMouseOpen)
          return;
      InputEvt(event);
    };
*/
  ...
  constructor.prototype = {
    play: function () {
      console.log("constructor.prototype.play().in")
      this.player.play();
    }
  };
  return constructor;
